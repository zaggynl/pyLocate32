...
class SettingsWindow(QtGui.QWidget, Ui_SettingsWindow):
    ...
    def __init__(self, win_parent=None):
        ...
        if os.path.isfile(fullconfpath):
            ...
            try:
                with open(fullconfpath, 'wt') as configfile:
                    config.write(configfile)
...
def searchThread():
    ...
    searchTimer.start()
    try:
        args = shlex.split("/usr/bin/locate "+str(main_window.lineEdit.text()) +" "+ "universal_newlines=True")
        #print(str(args))
        #universal_newlines=False is to open in textmode
        searchprocess = subprocess.Popen(args, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        output = searchprocess.communicate("universal_newlines=True")
        output2 = output[0].decode().split("\n")
        if output2 == ['']:
            haveSearchResults = False
        else:
            if  searchThreadAlive:
                ...
                viewmodel.removeRows(0, 1)
            for item in output2:
                #do something here otherwise segfault?
                #print("searchThreadAlive=%d" % searchThreadAlive)
                pass
